const {DWEB_MANIFEST_FILENAME, DWEB_HASH_REGEX} = require('./const')
...
function readManifest (vault, cb) {
  return maybe(cb, async function () {
    var data = await readFile(vault, DWEB_MANIFEST_FILENAME)
...
function writeManifest (vault, manifest, cb) {
  ...
  return writeFile(vault, DWEB_MANIFEST_FILENAME, JSON.stringify(manifest, null, 2), cb)
}
...
// helper to generate a new dweb.json object
function generateManifest (manifest = {}) {
  ...
  if (isString(author)) {
    if (author.startsWith('dweb://') || DWEB_HASH_REGEX.test(author)) {
      author = {url: author}
    } else {
      author = {name: author}
    }
  ...
  if (isObject(author)) {
    ...
    if (isString(author.url) && (author.url.startsWith('dweb://') || DWEB_HASH_REGEX.test(author.url))) {
