const {DPACK_MANIFEST_FILENAME, DPACK_HASH_REGEX} = require('./const')
const {maybe} = require('./common')
const {readFile} = require('./read')
const {writeFile} = require('./write')

// helper to read the manifest into an object
function readManifest (vault, cb) {
  return maybe(cb, async function () {
    var data = await readFile(vault, DPACK_MANIFEST_FILENAME)
...
function writeManifest (vault, manifest, cb) {
  manifest = generateManifest(manifest)
  return writeFile(vault, DPACK_MANIFEST_FILENAME, JSON.stringify(manifest, null, 2), cb)
}

// helper to write updates to a manifest object
function updateManifest (vault, updates, cb) {
  return maybe(cb, async function () {
    var manifest
    try {
      manifest = await readManifest(vault)
    } catch (e) {
      manifest = {}
    }
    Object.assign(manifest, generateManifest(updates))
    return writeManifest(vault, manifest)
  })
}

// helper to generate a new dpack.json object
function generateManifest (manifest = {}) {
  ...
  if (isString(author)) {
    if (author.startsWith('dweb://') || DPACK_HASH_REGEX.test(author)) {
      author = {url: author}
    } else {
      author = {name: author}
    }
  ...
  if (isObject(author)) {
    ...
    if (isString(author.url) && (author.url.startsWith('dweb://') || DPACK_HASH_REGEX.test(author.url))) {
