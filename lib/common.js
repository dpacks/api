...
const {NotFoundError, DestDirectoryNotEmpty} = require('@dbrowser/errors')

function toDBrowserError (err, info) {
  if (err.notFound || err.code === 'ENOENT' || err.code === 'ENOTDIR') {
    ...
    console.error(`dPack API: Unhandled error type from ${info}`, err)
...
async function findEntryByContentBlock (vault, block) {
  if (vault.metadata.length <= 0) {
    return
  }

  // do a binary search
  var lo = 1
  var hi = vault.metadata.length
  ...
  while (lo <= hi) {
    ...
    while (true) {
      ...
      entry = await new Promise(resolve =>
        vault.tree._getAndDecode(cursor, {}, (err, entry) => {
      ...
      if (entry.value) {
        st = vault.tree._codec.decode(entry.value)
        if (st.blocks !== 0) {
          break // found a file
        }
      }
