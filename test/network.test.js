...
test('download individual files', async t => {
  ...
  await dpackapi.download(dst, '/foo.txt')
  await dpackapi.download(dst, 'bar.data')
  await dpackapi.download(dst, '/subdir/foo.txt')
  await dpackapi.download(dst, 'subdir/bar.data')
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/foo.txt')), true)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/bar.data')), true)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/subdir/foo.txt')), true)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/subdir/bar.data')), true)
})
...
test('download a subdirectory', async t => {
  ...
  await dpackapi.download(dst, '/subdir')
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/foo.txt')), false)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/bar.data')), false)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/subdir/foo.txt')), true)
  t.deepEqual(isDownloaded(await dpackapi.stat(dst, '/subdir/bar.data')), true)
})
